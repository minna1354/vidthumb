...
export const metadata: Metadata = {
  ...
  description:
    "Complete guide to YouTube Shorts thumbnail dimensions, safe zones, character handling, file format, and best practices for 2026.",
  keywords: [
...
const content = `YouTube Shorts have become one of the most powerful discovery surfaces on the platform, with over 2 billion monthly logged-in viewers watching Shorts every day. Because Shorts are vertical, the thumbnail dimensions are completely different from regular YouTube videos. This guide covers everything you need to know about YouTube Shorts thumbnail size in 2026.
          ...
          if (para.startsWith("| ")) {
            ...
            return (
              <table key={i} className="w-full border-collapse my-4">
                <thead>
                  <tr>
                    {head.map((h, j) => <th key={j} className="border border-gray-200 px-3 py-2 bg-gray-50 text-left">{h}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {body.map((row, r) => (
                    <tr key={r}>
                      {row.map((c, j) => <td key={j} className="border border-gray-200 px-3 py-2 text-gray-700">{c}</td>)}
                    </tr>
                  ))}
                </tbody>
              </table>
